...
Shader "Study/07_study_specular"{
	...
	v2f ver(a2v verData)
	{
		...
		//相机位置			
		fixed3 viewDir = normalize(_WorldSpaceCameraPos.xyz - mul(unity_ObjectToWorld,verData.vertex).xyz);
		
		//镜面光颜色
		fixed3 specularColor = _LightColor0.rgb* _SpecularColor.rgb*pow(saturate(dot(specularDir, viewDir)), _Gloss);
